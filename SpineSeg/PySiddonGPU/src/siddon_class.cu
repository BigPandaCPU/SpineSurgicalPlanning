...
SiddonGpu::SiddonGpu(int *NumThreadsPerBlock,
	...
	m_DRRsizeX = DRRSize[0];
	m_DRRsizeY = DRRSize[1];

	m_movImgMemSize = MovSize[0] * MovSize[1] * MovSize[2] * sizeof(float);
	//printf("mov img size bytes:%zd", m_movImgMemSize);
	...
	//printf("\nbegin malloc mov img array.\n");
	HANDLE_ERROR(cudaMalloc((void**)&m_d_movImgArray, m_movImgMemSize));
...
**/
SiddonGpu::~SiddonGpu() {
    //std::cout << "Siddon object destruction: GPU memory cleared  begin\n" << std::endl;
	cudaFree(m_d_movImgArray);
	cudaFree(m_d_MovSize);
	cudaFree(m_d_MovSpacing);
	//std::cout << "Siddon object destruction: GPU memory cleared half \n" << std::endl;
	//printf("dtor %p\n", this); // in destructor

	cudaFree(d_drr_array);
	cudaFree(d_source);
	cudaFree(d_DestArray);
	std::cout << "Siddon object destruction: GPU memory cleared done \n" << std::endl;
							...
							float *drrArray) 
{
	...
	// launch kernel
	//std::cout<<" begin run cuda kernel\n"<<std::endl;
